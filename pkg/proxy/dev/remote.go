...
func NewRemoteProxy(
	...
	rproxy.checker.start(rproxy.stopCh) // start health checker
...
// ps: Because the organizer does not recommend using resourceusage label directly,
// we implemented a generic labelSelector-cachemgr
func (rp *RemoteProxy) modifyResponse(resp *http.Response) error {
	...
	if resp.StatusCode >= http.StatusOK && resp.StatusCode <= http.StatusPartialContent && info.Verb == "list" {
		...
		// cache labelSelector list resp(not include labelSelector is empty)
